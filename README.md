...
진단 CSV는 같은 `year + emp_num`에 여러 행이 있어도 됩니다. 여러 행은 한 구성원의 해당 연도 진단 근거로 묶어서 사용합니다. 단, `Category`와 `항목` 값은 분류 프롬프트, 지식 검색 컨텍스트, 결과 CSV에 넣지 않습니다.
...
diagnosis_row_count,diagnosis_teams,diagnosis_job_names,
...
- 입력 CSV의 `self_review`와 선택 입력인 `diagnosis_context`는 LLM에 근거 데이터로 전달합니다. `diagnosis_context`에는 diagnosis `team`, `진단 시 직무명`만 넣고 `Category`와 `항목` 값은 넣지 않습니다.
